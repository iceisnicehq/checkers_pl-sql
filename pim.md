...
- Обновление рейтинга после завершения матча: победитель получает 16*победы + 10*games_to_win, проигравший получает 16*победы - 10*games_to_win.
...
2. **Наполнение базы данных**
   ...
   Выполнить файлы наполнения из папки `tables/` после создания всех таблиц:
   - `13_insert_game_rules.sql` – наполнение правилами игры (выполнить после `02_game_rules.sql`)
   - `14_insert_seasons.sql` – создание начального сезона (выполнить после `03_seasons.sql`)
   - `15_insert_puzzles.sql` – наполнение начальными задачами (выполнить после `06_puzzles.sql`)
   - `16_insert_daily_puzzles.sql` – создание ежедневной задачи на сегодня (выполнить после `09_daily_puzzles.sql`)
...
7. **Создание автоматических заданий (schedulers)**
...
| 22 | Победа в матче (best of 3, победа 2-0) | Рейтинг победителя: +32 (2*16) + 30 (10*3) = +62, проигравшего: +0 - 30 = -30 → 0 (GREATEST) | |
| 23 | Победа в матче (best of 3, победа 2-1) | Рейтинг победителя: +32 (2*16) + 30 (10*3) = +62, проигравшего: +16 - 30 = -14 → 0 (GREATEST) | |
| 24 | Победа в матче (best of 5, победа 3-0) | Рейтинг победителя: +48 (3*16) + 50 (10*5) = +98, проигравшего: +0 - 50 = -50 → 0 (GREATEST) | |
| 25 | Победа в матче (best of 5, победа 3-2) | Рейтинг победителя: +48 (3*16) + 50 (10*5) = +98, проигравшего: +32 - 50 = -18 → 0 (GREATEST) | |
| 26 | Победа в матче (best of 7, победа 4-0) | Рейтинг победителя: +64 (4*16) + 70 (10*7) = +134, проигравшего: +0 - 70 = -70 → 0 (GREATEST) | |
| 27 | Победа в матче (best of 7, победа 4-3) | Рейтинг победителя: +64 (4*16) + 70 (10*7) = +134, проигравшего: +48 - 70 = -22 → 0 (GREATEST) | |
| 28 | Матч с ничьими (best of 3, победа 2-0, одна ничья) | Рейтинг победителя: +32 (2*16) + 30 (10*3) = +62, проигравшего: +0 - 30 = -30 → 0 (GREATEST) | |
| 29 | Матч с ничьими (best of 5, победа 3-1, одна ничья) | Рейтинг победителя: +48 (3*16) + 50 (10*5) = +98, проигравшего: +16 - 50 = -34 → 0 (GREATEST) | |
...
| 23 | После завершения матча | Рейтинги обновлены: победитель получает 16*победы + 10*games_to_win, проигравший получает 16*победы - 10*games_to_win | |


5.2.7.1 Проверка требования «Присоединение к матчу»

Проверка присоединения к матчу проходит в соответствии с таблицей 11. Присоединение к матчу происходит с помощью процедуры `join_match`. У процедуры 1 входной параметр: `p_match_id` (обязателен, тип NUMBER, должен существовать в таблице matches).

Таблица 11 – Состав входных параметров и ожидаемых результатов для присоединения к матчу
...
Проверка отмены игры проходит в соответствии с таблицей 12. Отмена игры происходит с помощью процедуры `cancel_game` (без параметров).

Таблица 12 – Состав условий и ожидаемых результатов для отмены игры
...
Проверка работы с задачами проходит в соответствии с таблицей 13. Работа с задачами происходит с помощью процедур: `show_puzzles` (3 параметра: `p_difficulty`, `p_puzzle_id`, `p_solution`), `show_my_puzzles` (1 параметр: `p_difficulty`), `create_puzzle` (5 параметров: `p_board_position`, `p_turn_to_move`, `p_moves_to_solve`, `p_difficulty_level`, `p_solution`), `delete_my_puzzle` (1 параметр: `p_puzzle_id`), `show_daily_puzzle` (без параметров).

Таблица 13 – Состав входных параметров и ожидаемых результатов для работы с задачами
...
Проверка режима наблюдателя проходит в соответствии с таблицей 14. Просмотр активной доски происходит с помощью процедуры `print_active_board`. У процедуры 3 входных параметра: `p_game_id` (необязателен, по умолчанию NULL, тип NUMBER), `p_username` (необязателен, по умолчанию NULL, тип VARCHAR2), `p_wait_for_turn` (необязателен, по умолчанию 'N', тип CHAR, допустимые значения: 'Y', 'N'). Выход из режима просмотра происходит с помощью процедуры `stop_spectating` (без параметров).

Таблица 14 – Состав входных параметров и ожидаемых результатов для режима наблюдателя
...
Проверка предложения и принятия ничьей проходит в соответствии с таблицей 15. Управление ничьей происходит с помощью процедуры `draw`. У процедуры 1 входной параметр: `p_action` (обязателен, тип CHAR, допустимые значения: 'O' - предложить, 'A' - принять, 'C' - отменить).

Таблица 15 – Состав входных параметров и ожидаемых результатов для предложения и принятия ничьей
...
Проверка сдачи в игре проходит в соответствии с таблицей 16. Сдача происходит с помощью процедуры `resign_game`. У процедуры 1 входной параметр: `p_resign_match` (необязателен, по умолчанию 'N', тип CHAR, допустимые значения: 'Y', 'N').

Таблица 16 – Состав входных параметров и ожидаемых результатов для сдачи в игре
...
Проверка представлений проходит в соответствии с таблицей 18.

Таблица 18 – Состав запросов и ожидаемых результатов для представлений
...
Проверка конкурентности проходит в соответствии с таблицей 21.

Таблица 21 – Состав тестов конкурентности и ожидаемых результатов
...
Проверка работы триггеров и автоматических заданий проходит в соответствии с таблицей 22.

Таблица 22 – Состав тестов триггеров и автоматических заданий и ожидаемых результатов
...
5.2.18 Проверка требования «Масштабируемость»

Проверка масштабируемости заключается в проверке работоспособности системы при больших объемах данных без деградации производительности ключевых запросов.

**Требования к масштабируемости:**
- Хранение ≥ 1 млн ходов без деградации производительности ключевых запросов
- Хранение ≥ 100 тыс партий без деградации производительности ключевых запросов
- Корректная работа индексов при больших объемах данных

**Метод проверки:**
1. Использовать скрипт `million_inserts.sql` для создания тестовых данных:
   - 100 тыс партий между игроками с ID 1 и 2
   - В каждой партии по 10 ходов
   - Итого: 1 млн ходов
2. Проверить производительность ключевых запросов:
   - Просмотр активных игр (`v_active_games`)
   - Просмотр истории игрока (`v_player_history`)
   - Просмотр протокола игры (`v_game_protocol`)
   - Просмотр рейтингов (`v_player_ratings`)
3. Убедиться, что время выполнения запросов остается приемлемым (зависит от конфигурации сервера)

**Примечание:** Скрипт `million_inserts.sql` создает тестовые данные для проверки масштабируемости. Выполнение скрипта может занять значительное время в зависимости от производительности сервера.
