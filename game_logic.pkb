CREATE OR REPLACE PACKAGE BODY C##CHECKERS_APP.game_logic AS
    ...
    -- (Все вспомогательные функции до get_printable_board остаются без изменений)
    -- ...
    ...
    BEGIN
        ...
        -- (Остальная часть процедуры остается без изменений)
        ...
        BEGIN
            ...
            DECLARE
                ...
                v_is_promotion_square BOOLEAN := (v_moving_piece = c_white_man AND v_end_row = 8) OR (v_moving_piece = c_black_man AND v_end_row = 1);
            BEGIN
                IF v_is_promotion_square THEN
        ...
        VALUES (p_game_id, v_move_count, v_player_id, p_move_notation, v_chosen_move.is_capture);
    ...
    PROCEDURE show_next_replay_move(p_game_id IN NUMBER) IS v_cursor SYS_REFCURSOR; v_game_id game_moves.game_id%TYPE; v_move_number game_moves.move_number%TYPE; v_turn_number NUMBER; v_username players.username%TYPE; v_move_notation game_moves.move_notation%TYPE; v_is_capture game_moves.is_capture%TYPE; v_move_ts game_moves.move_timestamp%TYPE; v_color_str VARCHAR2(10); BEGIN v_cursor := get_next_replay_move(p_game_id); FETCH v_cursor INTO v_game_id, v_move_number, v_turn_number, v_username, v_move_notation, v_is_capture, v_move_ts; IF v_cursor%FOUND THEN v_color_str := CASE WHEN MOD(v_move_number, 2) = 1 THEN '(Белые)' ELSE '(Черные)' END; DBMS_OUTPUT.PUT_LINE( 'Ход ' || v_turn_number || '. ' || RPAD(v_username, 20) || ' ' || RPAD(v_color_str, 10) || ' : ' || v_move_notation ); END IF; CLOSE v_cursor; EXCEPTION WHEN e_replay_session_not_started THEN DBMS_OUTPUT.PUT_LINE('[ВНИМАНИЕ] Сессия просмотра не начата. Вызовите game_logic.start_replay_session(' || p_game_id || ');'); WHEN OTHERS THEN IF v_cursor%ISOPEN THEN CLOSE v_cursor; END IF; RAISE; END show_next_replay_move;
...
FUNCTION get_printable_board(p_game_id IN NUMBER) RETURN CLOB IS
    ...
    BEGIN
        BEGIN
            SELECT
                g.board_position, g.status, g.current_turn, g.rule_id,
                CASE g.current_turn WHEN 'W' THEN g.player_white_id ELSE g.player_black_id END
            INTO
                v_board_position, v_status, v_current_turn, v_rule_id, v_active_player_id
            FROM games g
        ...
        -- === ВОЗВРАЩЕНА ВАША ЛОГИКА ПОДСВЕТКИ ===
        ...
        IF v_status = 'ACTIVE' AND v_viewer_player_id = v_active_player_id THEN
            ...
            IF v_legal_moves.COUNT > 0 AND v_legal_moves(1).is_capture = 'Y' THEN
                FOR i IN 1..v_legal_moves.COUNT LOOP
                    -- Подсвечиваем только конечную клетку каждого маршрута взятия
                    v_highlight_indices(v_legal_moves(i).path(v_legal_moves(i).path.LAST).end_idx) := TRUE;
                END LOOP;
            END IF;
        END IF;
        -- === КОНЕЦ БЛОКА ПОДСВЕТКИ ===
            ...
            FOR c IN 1..8 LOOP
                ...
                IF MOD(r + c, 2) != 0 THEN -- Только для темных (игровых) полей
                    ...
                    IF v_char = c_empty_field OR v_char = ' ' THEN
                        -- Проверка на подсветку для пустых полей
                        IF v_highlight_indices.EXISTS(v_linear_idx) THEN
                            DBMS_LOB.append(v_clob, '[.]'); -- Подсвеченное поле (точка)
                        ELSE
    ...
    FUNCTION get_possible_moves(p_game_id IN NUMBER) RETURN SYS_REFCURSOR IS
        ...
        v_player_color    CHAR(1);
        v_all_legal_moves t_move_list;
        -- ИСПОЛЬЗУЕМ ПУБЛИЧНЫЙ ТИП ИЗ СПЕЦИФИКАЦИИ ПАКЕТА
    ...
    BEGIN
        ...
        FOR i IN 1..v_all_legal_moves.COUNT LOOP
            v_notation_str := idx_to_notation(v_all_legal_moves(i).path(1).start_idx);
            FOR j IN 1..v_all_legal_moves(i).path.COUNT LOOP
                v_notation_str := v_notation_str ||
                                  CASE v_all_legal_moves(i).is_capture WHEN 'Y' THEN ':' ELSE '-' END ||
                                  idx_to_notation(v_all_legal_moves(i).path(j).end_idx);
            END LOOP;
            v_notations.EXTEND;
            -- ИСПОЛЬЗУЕМ КОНСТРУКТОР ПУБЛИЧНОГО ТИПА
